...
trait AbsoluteRDFWriter[Rdf <: RDF, M[_]: Functor, +S]:
   /** Write the triples of a graph to an java.io.Writer in a format that does not take relative
     * URLs.
     */
...
trait RDFrWriter[Rdf <: RDF, M[_]: Functor, +T]:
   ...
   /** write out the relative Graph to java.io.Writer Passing the graph as an iterator of Triples
     * allows one to specify the order of writing these out and also to relativise any URIs to be
     * written given the base. No need to specify the base, as we assume the graph is already
     * correctly set to relative.
     */
   def rgWrite(
       graph: rGraph[Rdf],
       wr: Writer,
...
trait AbsoluteRDFIterWriter[Rdf <: RDF, M[_], +S]:
   /** Write the triples of a graph to an java.io.Writer note: this Iterator output would be very
     * nice to have, but do existing implementations even provide this capability? note: (nice to
     * have apart from the fact that java.io.* is blocking)
     */
...
trait RDFIterWriter[Rdf <: RDF, M[_], +T]: // extends Writer[Rdf#Graph,M,T] {
   /** write out the Relative Triples from a graph to wr: java.io.Writer Passing the graph as an
     ...
     * relativise any URIs to be written note: this Iterator output would be very nice to have, but
     * do existing implementations even provide this capability? note: (nice to have apart from the
     * fact that java.io.* is blocking)
     */
   def write(
