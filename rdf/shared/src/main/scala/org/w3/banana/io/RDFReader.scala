...
trait AbsoluteRDFReader[Rdf <: RDF, M[_], +S]:
	 ...
	 * the parser to switch encoding and syntax for when the mime type
	 * was wrong. But that indicates that those should be called before 
	 * this.
	 */
	//def read(is: InputStream): M[Graph[Rdf]]

	/** Parses an RDF Graph from a java.io.Reader and a base URI.
	 * @param base the base URI to use, to resolve relative URLs
	 **/
...
trait RDFReader[Rdf <: RDF, M[_], +S]:
	...
	/**
	 * Parses an RDF Graph from an java.io.InputStream
	 * @param base URI in context M (Try, Future, ...)
	 */
...
/**
 * traits to be implemented by Syntaxes S that can contain relative URLs,
 * for Rdf implementations that can return relative graphs.
 *
 * @tparam Rdf types to use
 * @tparam M context in which result is returned
 * @tparam S syntax phantom type
 */
