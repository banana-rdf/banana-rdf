...
case class Patch[Rdf <: RDF](
  ...
  val whereVars: Set[Var[Rdf]] = where.map(_.block.vars).getOrElse(Set.empty)
  val insertVars: Set[Var[Rdf]] = insert.map(_.block.vars).getOrElse(Set.empty)
  val deleteVars: Set[Var[Rdf]] = delete.map(_.block.vars).getOrElse(Set.empty)
  ...
  assert(where.map(_.block.isTreePattern).getOrElse(true), "The BGP in the WHERE clause must be a Tree pattern")
