...
trait rURI[Rdf <: RDF](using ops: Ops[Rdf]):
   ...
   def resolveUri(uri: RDF.rURI[Rdf], base: ll.AbsoluteUrl): Try[(RDF.URI[Rdf], Boolean)] =
        ll.Uri.parseTry(stringValue(uri)).flatMap { llUri =>
           llUri match
             case _ : ll.Urn => Success((uri.asInstanceOf[RDF.URI[Rdf]], false)) // urns are always absolute
   ...
   extension (uri: RDF.rURI[Rdf])
     ...
     /**
       * Returns resolved URL, and whether the result is new.
       * If the URL is relative but cannot be resolved, be lenient and keep the original.
       * //todo: pass a logger
       * @param base the base URL to resolve against
       * @return (a resolved URL, true if the result is new)
       */
      def resolveUrlLenient(base: ll.AbsoluteUrl) :  (RDF.URI[Rdf], Boolean) =
      ...
      def resolve(base: ll.AbsoluteUrl): Try[(RDF.URI[Rdf], Boolean)] = resolveUri(uri, base)
