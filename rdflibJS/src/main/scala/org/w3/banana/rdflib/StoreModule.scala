...
import types.rdflib.rdflibStrings.*
import types.rdflib.tfTypesMod.*
import types.rdflib.typesMod.*
...
object storeMod {
	...
	trait IndexedFormula
		...
		def checkStatementList(
			sts: js.Array[Quad[QuadSubject, QuadPredicate, QuadObject, QuadGraph]],
			from: Double): Boolean | Unit = js.native
		...
		def copyTo(
			template: QuadSubject,
			target: QuadSubject,
			flags: js.Array[`two-direction` | delete_]): Unit = js.native
