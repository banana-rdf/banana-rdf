...
/** trait for Factories to create store
  *
  * There is too much diversity in preferences for store creation to hope to be able to create a
  * uniform interface for all of them at this time. Perhaps experience working with them will reveal
  * some commonality. So for the moment it is better to have a very simple trait that programmers
