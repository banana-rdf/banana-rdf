...
""")
    ...
    val names: Iterable[String] = solutionIterator(Await.result(asyncEngine.executeSelect(query), 1.second)) map { row => getNode(row, "name").flatMap(_.as[String]) getOrElse sys.error("") }
